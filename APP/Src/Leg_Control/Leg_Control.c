...
#define LEG_RS485_POST_RX_GUARD_US  20U
...
static void leg_dwt_init(void)
{
  CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;
  DWT->CYCCNT = 0U;
  DWT->CTRL |= DWT_CTRL_CYCCNTENA_Msk;
  dwt_cycles_per_us = SystemCoreClock / 1000000U;
  if (dwt_cycles_per_us == 0U) dwt_cycles_per_us = 1U;
}

static void leg_dwt_delay_us(uint32_t delay_us)
{
  uint32_t start = DWT->CYCCNT;
  uint32_t cycles = delay_us * dwt_cycles_per_us;
...
void Leg_Rx_Handler(Leg_HandlerTypeDef *hleg, uint16_t Size)
{
  ...
  if (motor == 0U && motor_is_online(leg, 1U)) {
    /* The motor has just released its RS485 reply.  Do not assert DE for the
       second motor until the documented bus turnaround guard has elapsed. */
    leg_dwt_delay_us(LEG_RS485_POST_RX_GUARD_US);
