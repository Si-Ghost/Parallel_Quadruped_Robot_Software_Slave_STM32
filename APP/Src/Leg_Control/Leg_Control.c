...
#define LEG_SERVICE_PERIOD_MS       1U
#define LEG_DEBUG_SERVICE_PERIOD_MS 50U
#define LEG_DMA_TRANSACTION_TIMEOUT_MS 2U
...
void Leg_Tx_Handler(Leg_HandlerTypeDef *hleg)
{
  ...
  HAL_StatusTypeDef ret = HAL_UARTEx_ReceiveToIdle_DMA(hleg->huartx,
                              leg_dma_rx[leg][motor], sizeof(MotorData_t));
  if (ret != HAL_OK) {
    dma_fail(leg, motor, ret);
    return;
  }
